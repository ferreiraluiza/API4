...
export async function inserirListaNegra(pessoa){
    ...
    const resposta = await con.query(comando, [pessoa.nome, pessoa.motivo, pessoa.dataVinganca, pessoa.notaOdio, pessoa.perdoado])
    let info = resposta[0]
...
export async function alterarListaNegra(pessoa, id){
    const comando = `
        ...
        `
        let resposta = await con.query(comando, [pessoa.nome, pessoa.motivo, pessoa.dataVinganca, pessoa.notaOdio, pessoa.perdoado, id])
